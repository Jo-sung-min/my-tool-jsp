	...
	<script>
		...
		function checkField() {
			...
			if(!inputs.id.value) { // name속성이 id인 요소의 value가 없으면 true
				alert("아이디를 입력하세요.");
				return false; // sumbit의 기본기능인 action에 지정한 pro페이지로 이동하는 기능을 막겠다..
			}
			if(!inputs.pw.value) { 
				alert("비밀번호를 입력하세요.");
				return false; 
		...
		function openConfirmId(inputForm) {
			...
			open(url, "confirmId", "width=500, height=500, toolbar=no, location=no, status=no, menubar=no, scrollbars=no, resizable=no"); 
		}
